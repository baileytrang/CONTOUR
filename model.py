...
from focus_models import Model, FocusSelector
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        if config.use_focus == "yes":
            self.lstm = nn.LSTM(config.emb_dim + config.focus_embed_size, config.hidden_dim,
                                num_layers=config.hidden_layers, batch_first=True, bidirectional=True)
        else:
        ...
        self.reduce_h = nn.Linear(
            config.hidden_dim * 2 * config.hidden_layers, config.hidden_dim)
        init_linear_wt(self.reduce_h)
        self.reduce_c = nn.Linear(
            config.hidden_dim * 2 * config.hidden_layers, config.hidden_dim)
        init_linear_wt(self.reduce_c)

    def forward(self, x, seq_lens, focus_mask=None):
        if config.use_focus == "yes":
            focus_embedding = self.focus_embed(focus_mask)
            enc_input = T.cat([x, focus_embedding], dim=2)
        else:
            enc_input = x
        packed = pack_padded_sequence(
            enc_input, seq_lens, batch_first=True, enforce_sorted=False)
...
class encoder_attention(nn.Module):
    ...
    def forward(self, spotlight_mask, st_hat, h, enc_padding_mask, sum_temporal_srcs):
        ...
        if config.spot_light == "yes":
            et = et*spotlight_mask
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.p_gen_linear = nn.Linear(
            config.hidden_dim * 5 + config.emb_dim, 1)
        ...
        if config.llint != 'no':
            self.Vllint = nn.Linear(
                config.hidden_dim, config.hidden_dim, bias=True)
            init_linear_wt(self.Vllint)

        init_linear_wt(self.V1)

    def forward(self, focus_mask, id_x, x_t, s_t, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, sum_temporal_srcs, prev_s, dec_lens):
        x = self.x_context(T.cat([x_t, ct_e], dim=1))
        s_t = self.lstm(x, s_t)

        dec_h, dec_c = s_t

        if config.llint != 'no' and prev_s == None and dec_lens != None:
            m = d.MultivariateNormal(
                T.zeros(config.hidden_dim), T.eye(config.hidden_dim))
            data = T.stack([m.sample() * dec_lens[i]
                            for i in range(config.batch_size)])
            data = data.cuda()
            dec_c = self.Vllint(data)

        st_hat = T.cat([dec_h, dec_c], dim=1)

        if config.spot_light == "yes":
            n = enc_batch_extend_vocab.size()[0]
            id_x = id_x.view(n, -1)
            spotlight_mask1 = (
                enc_batch_extend_vocab == id_x).long()
            spotlight_mask1 = T.cat(
                (T.zeros(n, 1).to(device), spotlight_mask1[:, :-1]), dim=1)

            spotlight_mask2 = (
                focus_mask == 1).long()
            spotlight_mask = spotlight_mask1 * spotlight_mask2 * config.lamda + 1
        else:
            spotlight_mask = None

        ct_e, attn_dist, sum_temporal_srcs = self.enc_attention(
            spotlight_mask, st_hat, enc_out, enc_padding_mask, sum_temporal_srcs)

        ct_d, prev_s = self.dec_attention(
            dec_h, prev_s)  # intra-decoder attention
...
class Model(nn.Module):
    def __init__(self):
        ...
        selector = FocusSelector(
            word_embed_size=config.emb_dim,
            num_layers=config.hidden_layers,
            enc_hidden_size=config.hidden_dim,
            dec_hidden_size=config.hidden_dim,
        ...
        self.mixture_embedding = nn.Embedding(config.n_mixture, config.emb_dim)
        self.focus_embed = nn.Embedding(
            3, config.focus_embed_size, padding_idx=-1)  # Binary
        self.encoder.focus_embed = self.focus_embed

        selector.add_embedding(self.embeds)
