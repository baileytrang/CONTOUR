...
class Example(object):
    ...
    def __init__(self, article, abstract_sentences, vocab):
        ...
        start_decoding = vocab.word2id(config.START_DECODING)
        stop_decoding = vocab.word2id(config.STOP_DECODING)
        abstract = ' '.join(abstract_sentences)

        article_words = article.split()
        abstract_words = abstract.split()

        if config.use_filter == "yes":
            if "cnn" in config.save_model_path or "xsum" in config.save_model_path:
                ignored_set = [",", ".", "'", "the", "to", "a", "and",
                               "of", "was", "for", "that", "is", "it", "with", "said"]
            elif "livedoor" in config.save_model_path:
                ignored_set = ["、", "の", "。", "に", "を", "が", "は",
                               "て", "た", "で", "と", "し", "・", "「", "」", "も", "(", ")", ":", ".", "-", "!"]
            elif "vntc" in config.save_model_path:
                ignored_set = [",", ".", "'", "-", "(", ")", ":", "và", "của", "là",
                               "có", "trong", "các", "được", "đó", "ấy", "những", "này", "như"]
        else:
            ignored_set = []
        ...
        for i in range(len(article_words)):
            if article_words[i] not in ignored_set and article_words[i] in self.original_abstract:
                self.focus_mask.append(1)
                self.focus_input.append(self.enc_input_extend_vocab[i])
            else:
                self.focus_mask.append(0)

        while len(self.focus_mask) < config.max_enc_steps:
            self.focus_mask.append(0)

        self.focus_mask = T.LongTensor(self.focus_mask)
        self.focus_input = T.FloatTensor(self.focus_input)
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list):
        ...
        self.focus_mask = np.zeros(
            (self.batch_size, max_enc_seq_len), dtype=np.float32)
        self.focus_input = [[] for _ in range(self.batch_size)]
        ...
        for i, ex in enumerate(example_list):
            ...
            self.focus_mask[i] = ex.focus_mask[:max_enc_seq_len]
            self.focus_input[i] = ex.focus_input
...
class Batcher(object):
    ...
    BATCH_QUEUE_MAX = config.max_epoch*config.total_doc//config.batch_size + 10
    ...
    def __init__(self, data_path, vocab, mode, batch_size, single_pass):
        ...
        self._example_queue = Queue.Queue(
            self.BATCH_QUEUE_MAX * self.batch_size)
    ...
    def text_generator(self, example_generator):
        while True:
            ...
            try:
                ...
                if "vntc" in config.save_model_path:
