...
class Beam(object):
    def __init__(self, start_id, end_id, unk_id, hidden_state, context):
        ...
        # beam,1; Initial score of beams = -30
        ...
        self.context = context.unsqueeze(0).repeat(
            config.beam_size, 1)  # beam, 2*n_hid
    ...
    def advance(self, prob_dist, hidden_state, context, sum_temporal_srcs, prev_s):
        ...
        best_scores, best_scores_id = T.topk(
            input=scores, k=config.beam_size, dim=0)
...
def beam_search(focus_mask, enc_hid, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, model, start_id, end_id, unk_id):

    batch_size = len(enc_hid[0])
    beam_idx = T.LongTensor(list(range(batch_size)))
    beams = [Beam(start_id, end_id, unk_id, (enc_hid[0][i], enc_hid[1][i]), ct_e[i])
             for i in range(batch_size)]  # For each example in batch, create Beam object
    # Index of beams that are active, i.e: didn't generate [STOP] yet
    n_rem = batch_size
    # Number of examples in batch that didn't generate [STOP] yet
    sum_temporal_srcs = None
    prev_s = None

    for t in range(65):
        x_t = T.stack(
            [beam.get_current_state() for beam in beams if beam.done ==
             False]  # remaining(rem),beam
        ...
        s_t = (dec_h, dec_c)
        enc_out_beam = enc_out[beam_idx].view(
            n_rem, -1).repeat(1, config.beam_size).view(-1, enc_out.size(1), enc_out.size(2))
        ...
        focus_mask_beam = focus_mask[beam_idx].repeat(
            1, config.beam_size).view(-1, focus_mask.size(1))

        final_dist, (dec_h, dec_c), ct_e, sum_temporal_srcs, prev_s = model.decoder(focus_mask_beam, id_x, x_t, s_t, enc_out_beam, enc_pad_mask_beam,
                                                                                    ct_e, extra_zeros_beam, enc_extend_vocab_beam, sum_temporal_srcs, prev_s, None)  # final_dist: rem*beam, n_extended_vocab
        ...
        if sum_temporal_srcs is not None:
            sum_temporal_srcs = sum_temporal_srcs.view(
                n_rem, config.beam_size, -1)  # rem, beam, n_seq
        ...
        for i in range(n_rem):
            ...
            beam.advance(final_dist[i], (dec_h[i], dec_c[i]),
                         ct_e[i], sum_temporal_srcs_i, prev_s_i)
            if beam.done == False:
                active.append(b)
